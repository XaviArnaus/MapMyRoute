...
class Writer {
    ...
    public function writeResults($result) {
        ...
        return file_put_contents($this->getEvents(true), $jsonized);
    }
    ...
    private function createEventsPath() {
        if (file_exists($this->getEvents(false))) return;

        if (!mkdir($this->getEvents(false), 0777, true)) {
            throw new RuntimeException("Could not create events dir. Check permissions!");
        }
    }

    private function getEvents($with_filename = true) {
